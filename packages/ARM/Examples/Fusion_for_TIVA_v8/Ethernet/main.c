 ...
 *                      /t0 ... /t7     will toggle RH0 to RH7 bit and return HTML main page
 ...
 * NOTES:
     - Turn on button press level for PORTB at SW3 (board specific);
     - Pull down PORTB pins;
     - Turn ON PORTH LEDs at SW4.
*/
...
"<HTML><HEAD><meta http-equiv=\"refresh\" content=\"3;url=http://10.101.14.36\"><meta charset=\"UTF-8\"></HEAD><BODY>\
<h1>Tiva Ethernet Mini Web Server</h1>\
<a href=/>Reload</a>\
<script src=/s></script>\
...
<tr><th colspan=3>PORTH</th></tr>\
<script>\
var str,i;\
str=\"\";\
for(i=0;i<8;i++)\
{str+=\"<tr><td bgcolor=yellow>LED H\"+i+\"</td>\";\
if(PORTH&(1<<i)){str+=\"<td bgcolor=red>ON\";}\
else {str+=\"<td bgcolor=#cccccc>OFF\";}\
...
const char  myIpAddr[4]  = {10, 101, 14, 36};                    // my IP address
const char  gwIpAddr[4]  = {10, 101, 14, 1};                     // gateway (router) IP address
const char  ipMask[4]    = {255, 255, 254, 0};                   // network mask (for example : 255.255.255.0)
const char  dnsIpAddr[4] = {10, 101, 14, 99};                    // DNS server IP address
...
unsigned int    Ethernet_Intern_UserTCP(unsigned char *remoteHost, unsigned int remotePort, unsigned int localPort, unsigned int reqLength, TEthInternPktFlags *flags)
        ...
        if(getRequest[5] == 's')                    // if request path name starts with s, store dynamic data in transmit buffer
                ...
                WordToStr(GPIO_PORTB_AHB_DATA, dyna);
                len += Ethernet_Intern_putString(dyna);
                len += Ethernet_Intern_putConstString(";");

                // add PORTH value (LEDs) to reply
                len += Ethernet_Intern_putConstString("var PORTH=");
                WordToStr(GPIO_PORTH_AHB_DATA, dyna);
                len += Ethernet_Intern_putString(dyna);
                len += Ethernet_Intern_putConstString(";");
        ...
        else if(getRequest[5] == 't')                                            // if request path name starts with t, toggle PORTH (LED) bit number that comes after
                {
                ...
                if(isdigit(getRequest[6]))                                       // if 0 <= bit number <= 9, bits 8 & 9 does not exist but does not matter
                        ...
                        GPIO_PORTH_AHB_DATA = GPIO_PORTH_AHB_DATA ^ bitMask;     // toggle PORTH with xor operator
                        }
                }
...
void main() {
  GPIO_Alternate_Function_Enable(&_GPIO_MODULE_ETHLED_K456_AHB);
  GPIO_Digital_Output(&GPIO_PORTH_AHB, _GPIO_PINMASK_ALL);
  GPIO_Digital_Input(&GPIO_PORTB_AHB, _GPIO_PINMASK_ALL);
  do {
    response = Ethernet_Intern_Init(myMacAddr, myIpAddr, _ETHERNET_FULLDUPLEX | _ETHERNET_SPD100);
  } while(response != 0);
